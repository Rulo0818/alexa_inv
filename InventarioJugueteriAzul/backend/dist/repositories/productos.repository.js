...
class ProductosRepository {
    ...
    async generarCodigo() {
        await (0, database_1.query)(`UPDATE secuencia_productos SET ultimo_numero = ultimo_numero + 1`);
        const resultado = await (0, database_1.query)(`SELECT ultimo_numero FROM secuencia_productos`);
    ...
    async buscarPorId(id) {
        const resultado = await (0, database_1.query)(`SELECT p.*, c.nombre as nombre_categoria, c.emoji as emoji_categoria
       FROM productos p
       INNER JOIN categorias c ON p.id_categoria = c.id
       WHERE p.id = ?`, [id]);
    ...
    async existeCodigo(codigo) {
        const resultado = await (0, database_1.query)(`SELECT COUNT(*) as total FROM productos WHERE codigo = ?`, [codigo]);
        return resultado[0].total > 0;
    }
    // Crear producto
    async crear(datos, codigo) {
        const resultado = await (0, database_1.query)(`INSERT INTO productos 
    ...
    async eliminar(id) {
        await (0, database_1.query)(`UPDATE productos SET activo = 0 WHERE id = ?`, [id]);
    }
    // Obtener estadísticas de inventario
    async obtenerEstadisticas() {
        // Total productos
        const totalResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM productos WHERE activo = 1`);
        // Valor inventario
        const valorResult = await (0, database_1.query)(`SELECT SUM(precio_de_compra * stock) as valor FROM productos WHERE activo = 1`);
        // Stock bajo
        const stockBajoResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM productos 
       WHERE activo = 1 AND stock < stock_minimo AND stock > 0`);
        // Agotados
        const agotadosResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM productos WHERE activo = 1 AND stock = 0`);
        // Por categoría
        const porCategoriaResult = await (0, database_1.query)(`SELECT c.id as id_categoria, c.nombre as nombre_categoria, 
       ...
       ORDER BY cantidad DESC`);
        return {
            ...
            por_categoria: porCategoriaResult.map((cat) => ({
