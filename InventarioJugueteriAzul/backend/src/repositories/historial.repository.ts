...
export interface ListarHistorialQuery {
  fecha_inicio?: string;
  fecha_fin?: string;
  id_usuario?: number;
  tipo_accion?: string;
...
export class HistorialRepository {
  ...
  async listar(filtros: ListarHistorialQuery): Promise<AccionHistorial[]> {
    let sql = `
  ...
  async obtenerEstadisticasHoy(): Promise<EstadisticasHistorial> {
    const hoy = new Date().toISOString().split('T')[0];

    const [accionesResult, ventasResult, cambiosResult, usuariosResult] = await Promise.all([
      query(
        `SELECT COUNT(*) as total FROM historial_acciones WHERE DATE(fecha_creacion) = ?`,
        [hoy]
      ),
      query(
        `SELECT COUNT(*) as total FROM historial_acciones 
         WHERE tipo_accion = 'venta' AND DATE(fecha_creacion) = ?`,
        [hoy]
      ),
      query(
        `SELECT COUNT(*) as total FROM historial_acciones 
      ...
      query(
        `SELECT COUNT(DISTINCT id_usuario) as total FROM historial_acciones WHERE DATE(fecha_creacion) = ?`,
        [hoy]
      )
    ]);
