...
export class VentasRepository {
  ...
  async registrarVenta(
    ...
    const resultado = await query(
      `INSERT INTO ventas (id_empleado, total, metodo_pago, fecha, hora, cancelada)
       VALUES (?, ?, ?, ?, ?, 0)`,
      [id_empleado, total, metodo_pago, fecha, hora]
    );
    ...
    for (const prod of productos) {
      await query(
        `INSERT INTO detalle_ventas (id_venta, id_producto, cantidad, precio_unitario, subtotal)
         VALUES (?, ?, ?, ?, ?)`,
        [id_venta, prod.id_producto, prod.cantidad, prod.precio_unitario, prod.cantidad * prod.precio_unitario]
      );
    }
  }

  // Listar ventas
  async listar(filtros: ListarVentasQuery): Promise<VentaCompleta[]> {
    let sql = `
      SELECT v.*, 
  ...
  // Buscar venta por ID
  async buscarPorId(id: number): Promise<VentaCompleta | null> {
    const resultado = await query(
      `SELECT v.*, 
              u.nombre as nombre_empleado, 
              u.apellido as apellido_empleado
  ...
  async cancelar(
    ...
    await query(
      `UPDATE ventas 
       ...
       WHERE id = ?`,
      [motivo, cancelada_por, id]
    );
  }

  // Verificar si venta puede cancelarse (máximo 7 días)
  async puedeCancelarse(id: number): Promise<boolean> {
    const resultado = await query(
      `SELECT DATEDIFF(NOW(), fecha_creacion) as dias FROM ventas WHERE id = ?`,
      [id]
    );
  ...
  async obtenerEstadisticas(filtros: ListarVentasQuery): Promise<EstadisticasVentas> {
    ...
    const ventasPorEmpleado = await query(ventasPorEmpleadoSql, params);
    ...
    return {
      ...
      ventas_por_empleado: ventasPorEmpleado.map((e: any) => ({
        nombre: e.nombre,
        apellido: e.apellido,
